    ...
    if(_b.college_email.toLowerCase().split("@")[1] !== Constant.emailSuffix) {
        ...
        try {
            const user = await User.create({
                ...
                degree: _b.degree,
                permission: 'student',
                department: _b.branch,
                passout_batch: _b.year,
                cgpa: _b.cgpa,
                contact_no: _b.contact_no,
                password: _b.password,
                
                ...Constant.studentData
            });

            res.status(200).json({ success : true, message : 'User successfully added. Please complete your profile by logging in'})

            // const notif = await SMS.sendDM(_b, 'addCoordinator');
        }
        catch(err) {
            console.error(err);
            res.status(200).json({ success : false, message : 'Something went wrong! ID or email already present'});
        }
    }
}
    ...
    if(!_b.college_id || !_b.password) {
        ...
        try {
            ...
            if(!user) {
                ...
                let validPassword = await user.comparePassword(_b.password);
...
exports.getUsers = async (req, res) => {
    ...
    const userData = await User.find({permission:'student'}).lean();
  //  console.log(userData)
    if(!userData) {
        
        res.status(200).json({ success : false, message : 'Incorrect.'})
    } else {
        
        res.status(200).json({ success : true, studentData : userData})
    }
}
